...
export async function GET() {
  const db = await readDB('support.json', { faqData: defaultFaqData, contactInfo: defaultContactInfo });
...
export async function PUT(request) {
  try {
    ...
    if (updateData.faqData) db.faqData = updateData.faqData;
    if (updateData.contactInfo) db.contactInfo = updateData.contactInfo;
    
    await writeDB('support.json', db);
    return Response.json({ success: true });
  } catch (error) {
    return Response.json({ error: 'Failed to update support data' }, { status: 500 });
  }
}
