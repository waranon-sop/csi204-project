...
const LOOK_1_ITEMS = [
  { id: 'l1_1', name: '1992 Vintage Denim', color: 'Blue', hex: '#2A5C91', size: 'M (32")', price: 1200 },
  { id: 'l1_2', name: 'Midnight Silk Circuit Shirt', color: 'Cream', hex: '#F9F6F0', size: 'L', price: 800 },
  { id: 'l1_3', name: 'Vintage Leather Satchel', color: 'Tan', hex: '#C28E5F', size: 'OS', price: 1500 },
  { id: 'l1_4', name: 'Classic Chelsea Boots', color: 'Brown', hex: '#633B26', size: '40', price: 2100 },
  { id: 'l1_5', name: 'Gold Pendant Necklace', color: 'Gold', hex: '#D4AF37', size: 'OS', price: 450 },
  { id: 'l1_6', name: 'Classic Leather Watch', color: 'Brown/Gold', hex: '#8B5A2B', size: 'OS', price: 1800 },
];

const LOOK_2_ITEMS = [
  { id: 'l2_1', name: 'Reconstructed Chore Coat', color: 'Olive', hex: '#556B2F', size: 'L', price: 1500 },
  { id: 'l2_2', name: 'Vintage Corduroy Trousers', color: 'Brown', hex: '#5C4033', size: '34', price: 950 },
  { id: 'l2_3', name: 'Vintage Suede Desert Boots', color: 'Brown', hex: '#8B5A2B', size: '42', price: 1200 },
  { id: 'l2_4', name: 'Canvas Tote Bag', color: 'Cream', hex: '#F5F5F0', size: 'OS', price: 500 },
  { id: 'l2_5', name: 'Classic Leather Belt', color: 'Brown', hex: '#5C3A21', size: '34', price: 600 },
  { id: 'l2_6', name: 'Vintage Field Watch', color: 'Brown/Gold', hex: '#B8860B', size: 'OS', price: 2200 },
  { id: 'l2_7', name: 'Canvas Utility Pouch', color: 'Olive', hex: '#6B705C', size: 'OS', price: 450 },
];
...
export default function ProductCollection() {
  ...
  const [look2Selection, setLook2Selection] = useState(LOOK_2_ITEMS.map(i => i.id));
  ...
  const toggleLook1 = (id) => setLook1Selection(prev => prev.includes(id) ? prev.filter(i => i !== id) : [...prev, id]);
  const toggleLook2 = (id) => setLook2Selection(prev => prev.includes(id) ? prev.filter(i => i !== id) : [...prev, id]);

  const look1Total = LOOK_1_ITEMS.filter(i => look1Selection.includes(i.id)).reduce((acc, curr) => acc + curr.price, 0);
  const look2Total = LOOK_2_ITEMS.filter(i => look2Selection.includes(i.id)).reduce((acc, curr) => acc + curr.price, 0);
  ...
  const handleAddLookToCart = async (lookItems, selectedIds, lookNum) => {
    ...
    const itemsToAdd = lookItems.filter(item => selectedIds.includes(item.id));
      ...
      <section id="collection" className="max-w-7xl mx-auto px-6 sm:px-8 py-20 space-y-10 animate-fade-up delay-100 bg-[#F9F8F6]">
        ...
        <div className="mb-8">
          <div className="flex flex-col md:flex-row md:items-center justify-between border-y border-[#EAE5DB] bg-white relative z-40">
            ...
            <div className="relative h-full flex">
              ...
              >
                Size
                <svg className={`w-3.5 h-3.5 transition-transform ${openDropdown === 'size' ? 'rotate-180' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M19 9l-7 7-7-7" />
                </svg>
              </button>
              {openDropdown === 'size' && (
                <div className="absolute top-full left-0 w-48 bg-white border border-[#EAE5DB] border-t-0 shadow-lg z-50 py-2">
                  {['L', 'M', 'M/L', 'S', 'XL', 'XL/XXL', 'XS', 'XS/S'].map(size => (
                    <label key={size} className="flex items-center gap-3 px-4 py-2 hover:bg-[#F9F8F6] cursor-pointer">
                      <div className="relative flex items-center justify-center">
                        <input type="checkbox" checked={selectedFilters.includes(size)} onChange={() => toggleFilter(size)} className="peer appearance-none w-4 h-4 border border-[#D1D1D1] rounded-sm checked:bg-[#2D2D2A] checked:border-[#2D2D2A] cursor-pointer transition-colors" />
                      ...
                      <span className="text-[13px] text-[#2D2D2A]">{size}</span>
                    </label>
                  ))}
                </div>
              )}
            ...
            <div className="relative h-full flex">
              ...
              >
                Category
                <svg className={`w-3.5 h-3.5 transition-transform ${openDropdown === 'category' ? 'rotate-180' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M19 9l-7 7-7-7" />
                </svg>
              </button>
              {openDropdown === 'category' && (
                <div className="absolute top-full left-0 w-56 bg-white border border-[#EAE5DB] border-t-0 shadow-lg z-50 py-2">
                  {['T-Shirts', 'Shirts & Blouses', 'Sweaters & Knitwear', 'Jackets & Coats', 'Jeans', 'Trousers & Pants', 'Skirts', 'Dresses', 'Other'].map(cat => (
                    <label key={cat} className="flex items-center gap-3 px-4 py-2 hover:bg-[#F9F8F6] cursor-pointer">
                      <div className="relative flex items-center justify-center">
                        <input type="checkbox" checked={selectedFilters.includes(cat)} onChange={() => toggleFilter(cat)} className="peer appearance-none w-4 h-4 border border-[#D1D1D1] rounded-sm checked:bg-[#2D2D2A] checked:border-[#2D2D2A] cursor-pointer transition-colors" />
                        <svg className="absolute w-3 h-3 text-white pointer-events-none opacity-0 peer-checked:opacity-100 transition-opacity" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M5 13l4 4L19 7" />
                        </svg>
                      </div>
                      <span className="text-[13px] text-[#2D2D2A]">{cat}</span>
                    </label>
                  ))}
                </div>
              )}
            ...
            <div className="relative h-full flex">
              ...
              >
                Color
                <svg className={`w-3.5 h-3.5 transition-transform ${openDropdown === 'color' ? 'rotate-180' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M19 9l-7 7-7-7" />
                </svg>
              </button>
              {openDropdown === 'color' && (
                <div className="absolute top-full left-0 w-48 bg-white border border-[#EAE5DB] border-t-0 shadow-lg z-50 py-2">
                  {[
                    ...
                    <label key={color.name} className="flex items-center gap-3 px-4 py-2 hover:bg-[#F9F8F6] cursor-pointer">
                      <div className="relative flex items-center justify-center">
                        <input type="checkbox" checked={selectedFilters.includes(color.name)} onChange={() => toggleFilter(color.name)} className="peer appearance-none w-4 h-4 border border-[#D1D1D1] rounded-sm checked:bg-[#2D2D2A] checked:border-[#2D2D2A] cursor-pointer transition-colors" />
                        <svg className="absolute w-3 h-3 text-white pointer-events-none opacity-0 peer-checked:opacity-100 transition-opacity" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M5 13l4 4L19 7" />
                        </svg>
                      </div>
                      <div className="w-3 h-3 rounded-full border border-[#D1D1D1] shrink-0" style={{ backgroundColor: color.hex }}></div>
                      <span className="text-[13px] text-[#2D2D2A]">{color.name}</span>
                    </label>
                  ))}
                </div>
              )}
          ...
          <div className="h-14 flex items-center border-t md:border-t-0 md:border-l border-[#EAE5DB] shrink-0 w-full md:w-auto relative">
            ...
            >
              ...
              <svg className={`w-3.5 h-3.5 transition-transform ${openDropdown === 'price' ? 'rotate-180' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M19 9l-7 7-7-7" />
              </svg>
            </button>
            {openDropdown === 'price' && (
                ...
                <div className="grid grid-cols-2 gap-y-5 gap-x-8">
                  {[
                    ...
                    <label key={range} className="flex items-center gap-3 cursor-pointer group">
                      ...
                      <span className={`text-[13px] transition-colors ${selectedPriceRange === range ? 'text-[#2D2D2A] font-bold' : 'text-[#8B8B88] group-hover:text-[#5C5C5A]'}`}>{range}</span>
                    </label>
                  ))}
                </div>
              </div>
        ...
        {/* Active Filters Row */}
          {(selectedFilters.length > 0 || selectedPriceRange !== 'ทั้งหมด') && (
            <div className="flex flex-wrap items-center gap-2 py-3 border-b border-[#EAE5DB] bg-[#F9F8F6]/50 px-6">
              <button 
                onClick={() => { setSelectedFilters([]); setSelectedPriceRange('ทั้งหมด'); }} 
              ...
              {selectedPriceRange !== 'ทั้งหมด' && (
                ...
                >
                  {selectedPriceRange}
                  <svg className="w-3 h-3 text-[#A0A09F] group-hover:text-[#2D2D2A] transition-colors" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <div className="pt-24 pb-10 border-t border-[#EAE5DB] mt-20">
          <div className="text-center mb-12">
            <h2 className="font-serif text-3xl font-bold text-[#2D2D2A]">LOOKBOOK</h2>
          ...
          <div className="flex flex-col gap-24 lg:gap-32">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-16 items-center">
              <div className="relative aspect-[4/5] w-full overflow-hidden">
                <Image src="/styling_1.png" alt="Look 01" fill className="object-cover" />
              </div>
              
              <div className="flex flex-col justify-center">
                <span className="text-[11px] font-bold tracking-[0.2em] uppercase text-[#8B8B88] mb-4">Daily Match</span>
                <h3 className="font-serif text-3xl md:text-4xl font-bold text-[#2D2D2A] mb-12">Look 01</h3>
                
                <div className="space-y-6 mb-12 max-h-[300px] overflow-y-auto pr-2 no-scrollbar">
                  {LOOK_1_ITEMS.map((item, idx) => (
                    <div key={item.id} onClick={() => isLook1Selecting && toggleLook1(item.id)} className={`flex items-start gap-4 ${isLook1Selecting ? 'cursor-pointer group' : ''} ${idx !== LOOK_1_ITEMS.length - 1 ? 'border-b border-[#EAE5DB] pb-5' : ''}`}>
                      {isLook1Selecting && (
                        <div className="relative flex items-center justify-center mt-1 shrink-0">
                          <input type="checkbox" checked={look1Selection.includes(item.id)} readOnly className="peer appearance-none w-5 h-5 border-2 border-[#D1D1D1] rounded-sm checked:bg-[#2D2D2A] checked:border-[#2D2D2A] transition-colors cursor-pointer" />
                          <svg className="absolute w-3.5 h-3.5 text-white pointer-events-none opacity-0 peer-checked:opacity-100 transition-opacity" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M5 13l4 4L19 7" /></svg>
                        </div>
                      )}
                      <div className="flex-1">
                        <h4 className={`text-[14px] font-bold transition-colors ${!isLook1Selecting || look1Selection.includes(item.id) ? 'text-[#2D2D2A]' : 'text-[#A0A09F] line-through'}`}>{item.name}</h4>
                        <div className={`text-[12px] font-medium tracking-wide mt-1.5 transition-colors flex flex-wrap items-center gap-2 ${!isLook1Selecting || look1Selection.includes(item.id) ? 'text-[#5C5C5A]' : 'text-[#A0A09F]'}`}>
                          <div className="flex items-center gap-1.5">
                            Color: <span className="w-2.5 h-2.5 rounded-full border border-black/10 shadow-sm" style={{ backgroundColor: item.hex }} /> {item.color}
                          </div>
                ...
                {!isLook1Selecting ? (
                  ...
                  >
                    SHOP THIS LOOK (THB {LOOK_1_ITEMS.reduce((sum, item) => sum + item.price, 0).toLocaleString()})
                    <svg className="w-4 h-4 group-hover:translate-x-1 transition-transform" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M14 5l7 7m0 0l-7 7m7-7H3" /></svg>
                  </button>
                ) : (
                  <button 
                    disabled={look1Selection.length === 0 || isLook1Adding}
                    onClick={() => handleAddLookToCart(LOOK_1_ITEMS, look1Selection, 1)}
                  ...
                  >
                    ...
                    {!isLook1Adding && look1Selection.length > 0 && <svg className="w-4 h-4 group-hover:translate-x-1 transition-transform" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4v16m8-8H4" /></svg>}
                  </button>
                )}
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-16 items-center">
              <div className="relative aspect-[4/5] w-full overflow-hidden lg:order-2">
                <Image src="/styling_2.png" alt="Look 02" fill className="object-cover" />
              </div>
              
              <div className="flex flex-col justify-center lg:order-1">
                <span className="text-[11px] font-bold tracking-[0.2em] uppercase text-[#8B8B88] mb-4">Daily Match</span>
                <h3 className="font-serif text-3xl md:text-4xl font-bold text-[#2D2D2A] mb-12">Look 02</h3>
                
                <div className="space-y-6 mb-12 max-h-[300px] overflow-y-auto pr-2 no-scrollbar">
                  {LOOK_2_ITEMS.map((item, idx) => (
                    <div key={item.id} onClick={() => isLook2Selecting && toggleLook2(item.id)} className={`flex items-start gap-4 ${isLook2Selecting ? 'cursor-pointer group' : ''} ${idx !== LOOK_2_ITEMS.length - 1 ? 'border-b border-[#EAE5DB] pb-5' : ''}`}>
                      {isLook2Selecting && (
                        <div className="relative flex items-center justify-center mt-1 shrink-0">
                          <input type="checkbox" checked={look2Selection.includes(item.id)} readOnly className="peer appearance-none w-5 h-5 border-2 border-[#D1D1D1] rounded-sm checked:bg-[#2D2D2A] checked:border-[#2D2D2A] transition-colors cursor-pointer" />
                          <svg className="absolute w-3.5 h-3.5 text-white pointer-events-none opacity-0 peer-checked:opacity-100 transition-opacity" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M5 13l4 4L19 7" /></svg>
                        </div>
                      )}
                      <div className="flex-1">
                        <h4 className={`text-[14px] font-bold transition-colors ${!isLook2Selecting || look2Selection.includes(item.id) ? 'text-[#2D2D2A]' : 'text-[#A0A09F] line-through'}`}>{item.name}</h4>
                        <div className={`text-[12px] font-medium tracking-wide mt-1.5 transition-colors flex flex-wrap items-center gap-2 ${!isLook2Selecting || look2Selection.includes(item.id) ? 'text-[#5C5C5A]' : 'text-[#A0A09F]'}`}>
                          <div className="flex items-center gap-1.5">
                            Color: <span className="w-2.5 h-2.5 rounded-full border border-black/10 shadow-sm" style={{ backgroundColor: item.hex }} /> {item.color}
                          </div>
                          <span className="text-[#EAE5DB]">|</span>
                ...
                {!isLook2Selecting ? (
                  <button 
                    ...
                    className="bg-[#2D2D2A] text-white text-[12px] font-bold tracking-[0.15em] uppercase py-5 px-8 w-full hover:bg-[#1A1A1A] transition-colors rounded-sm flex items-center justify-center gap-3 group"
                  >
                    SHOP THIS LOOK (THB {LOOK_2_ITEMS.reduce((sum, item) => sum + item.price, 0).toLocaleString()})
                    <svg className="w-4 h-4 group-hover:translate-x-1 transition-transform" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M14 5l7 7m0 0l-7 7m7-7H3" /></svg>
                  </button>
                ) : (
                  <button 
                    disabled={look2Selection.length === 0 || isLook2Adding}
                    onClick={() => handleAddLookToCart(LOOK_2_ITEMS, look2Selection, 2)}
                    className="bg-[#5F6B4E] text-white text-[12px] font-bold tracking-[0.15em] uppercase py-5 px-8 w-full hover:bg-[#4A533D] transition-colors rounded-sm flex items-center justify-center gap-3 group disabled:bg-[#D1D1D1] disabled:text-[#8B8B88] disabled:cursor-not-allowed"
                  >
                    {isLook2Adding ? (
                      <>
                    ...
                    {!isLook2Adding && look2Selection.length > 0 && <svg className="w-4 h-4 group-hover:translate-x-1 transition-transform" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4v16m8-8H4" /></svg>}
                  </button>
                )}
              </div>
            </div>
